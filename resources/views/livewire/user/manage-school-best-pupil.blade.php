<div class="w-full max-w-6xl py-3 px-4 mx-auto shadow-3 shadow-sky-500 rounded-xl my-2" >
    ...
    <div class="mt-10 flex justify-center flex-col items-center">
        <div class="w-full bg-black/80 rounded-lg border-amber-600 border-2 shadow-lg shadow-gray-600 mb-11">
            <h6 class="text-center p-2 letter-spacing-1 font-bold text-lg uppercase text-amber-600 my-2">
            ...
            <div class="w-full">
                ...
                <div class="">
                    <div class="bg-green-400 shadow-lg pb-4 rounded-4xl flex flex-col justify-center">
                        ...
                        <h6 class="text-lg font-semibold letter-spacing-1 text-center text-green-900 p-2 pb-6">
                            L'apprenant {{ $gender == 'Feminin' ? 'e' : '' }} 
                            <a href="{{$school->to_profil_route()}}#school_bests_pupils">
                            <span class="underline underline-offset-4 text-black">
                                {{ $pupil_name }}
                            </span> 
                            </a>
                            a été ajouté {{ $gender == 'Feminin' ? 'e' : '' }} à la liste des meilleurs élèves avec succès!
                        </h6>
                ...
                <div class=" bg-black/80 border border-sky-500 rounded-lg shadow-2xl">
                    ...
                    <div class="w-full flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200">
                        <div class="text-sm sm:text-lg font-semibold text-lime-500 letter-spacing-1 w-full">
                            @if($school_best_pupil)
                                <div class="w-full">
                                    <div class="w-full mx-auto">
                                        <div class="flex justify-end items-center mx-auto">
                                            <img class="h-14 w-14 md:h-20 md:w-20 rounded-full object-cover border-2 border-amber-500 cursor-pointer shadow-sm shadow-gray-400" src="{{url('storage', $school_best_pupil->image_path)}}" alt="Image du meilleur {{$school_best_pupil->exam}} de l'école">
                                            <div  class=" items-center cursor-pointer bg-black/90 text-white inline-flex p-3 text-center  justify-center bg-opacity-70 opacity-80 group-hover:text-amber-500 transition-all duration-200">
                                                <div class="flex space-x-4 cursor-pointer text-center letter-spacing-2 text-xs">
                                                    photo de  
                                                    <span class="text-center text-amber-500 mx-1.5">  {{  $school_best_pupil->pupil_name }} </span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <form wire:submit.prevent="save" class="p-4 md:p-5">
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2 items-center grid grid-cols-4 gap-1.5">
                                ...
                                <div class="col-span-3 grid gap-2 mb-4 grid-cols-6">
                                    <div class="col-span-4">
                                        <label for="pupil_name" class="block mb-2 text-sm text-amber-400 font-medium ">Nom et prénoms </label>
                                    ...
                                    <div class="col-span-2">
                                        <label for="gender" class="block mb-2 text-sm text-amber-400 font-medium ">Sexe</label>
                                        <select wire:model.live='gender' id="gender" class="border border-sky-400 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:border-gray-500 dark:placeholder-gray-400 text-gray-400 dark:focus:ring-primary-500 dark:focus:border-primary-500 bg-transparent">
                                            <option class="text-sm text-white py-1.5 bg-black" >Selectionner la gender</option>
                                            @foreach ($genders as $g => $gd)
                                                <option class="text-white py-1.5 bg-black px-2.5" value="{{$gd}}">{{ $gd }}</option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="col-span-2 grid gap-4 mb-4 grid-cols-6 border border-gray-500 rounded-md p-2">
                                <div class="col-span-3">
                                    <label for="exam" class="block mb-2 text-sm text-amber-400 font-medium ">L'examen</label>
                                    <input wire:model.live='exam' type="text" name="exam" id="exam" class="bg-transparent border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5   dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Renseigner l'examen" >
                                ...
                                <div class="col-span-3">
                                    ...
                                    <select wire:model.live='year' id="year" class="border border-sky-400 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:border-gray-500 dark:placeholder-gray-400 text-gray-400 dark:focus:ring-primary-500 dark:focus:border-primary-500 bg-transparent">
                                        <option class="text-sm text-white py-1.5 bg-black" >Selectionner l'année</option> 
                                        @foreach (getYears() as $ky => $y)
                                            <option class="text-white py-1.5 bg-black px-2.5" value="{{$y}}">{{ $y }}</option>
                                        @endforeach
                                    </select>
                                    ...
                                    <table class="min-w-full divide-y text-xs sm:text-sm letter-spacing-1 divide-gray-200 border">
                                        ...
                                        <tbody class="divide-y text-gray-200 divide-gray-200">
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td title="Cliquer pour éditer ce détail" class="px-2 py-1 whitespace-nowrap">
                                                    <div wire:click="editDetailByClick('{{$sub}}', '{{$mk}}')" class="cursor-pointer">
                                                        {{ $sub }}
                                                    </div>
                                                </td>
                                                <td title="Cliquer pour éditer ce détail" class="px-2 py-1 whitespace-nowrap">
                                    ...
                                    <table class="min-w-full divide-y text-xs sm:text-sm letter-spacing-1 divide-gray-200 border">
                                        ...
                                        <tbody class="divide-y text-gray-200 divide-gray-200">
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td title="Cliquer pour éditer ce record" class="px-2 py-1 whitespace-nowrap">
                                                    <div wire:click="editRankByClick('{{$zn}}', '{{$rk}}')" class="cursor-pointer">
                                                        {{ $zn }}
                                                    </div>
                                                </td>
                                                <td title="Cliquer pour éditer ce record" class="px-2 py-1 whitespace-nowrap">
    ...
    <h6 class="text-center flex flex-col text-amber-400 letter-spacing-1 font-semibold my-3">
        ...
        @else
        <a class="bg-gray-900 hover:bg-gray-900 text-green-500 hover:underline underline-offset-2 rounded-lg p-2 py-3 my-2 mb-3 border border-amber-500" href="{{$school->to_profil_route()}}#school_bests_pupils">
            <span class="fas fa-school"></span>
            <span>
                Retour à la page profil de l'école
            </span>
        </a>
        @endif
    </h6>
