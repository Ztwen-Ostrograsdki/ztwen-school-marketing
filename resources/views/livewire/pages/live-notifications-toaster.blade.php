    ...
    @if(auth_user() && $toasters_data)
        @foreach ($toasters_data as $key => $toaster)
            @php
                $name = $toaster->id;
            @endphp
        <div wire:key="toaster-page-{{getRandom(253435, 7736535534)}}" id="toaster-{{$toaster->id}}" class="flex my-2 items-center w-full max-w-xs p-2 px-3 mb-4 text-yellow-500 bg-gray-800 rounded-lg shadow-sm border border-green-400 opacity-85 transition-opacity" role="alert" >
